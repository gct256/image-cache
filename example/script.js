...
UUID = function (overwrittenUUID) {
  ...
  (function () {
    ...
    } else if (typeof commonjsRequire !== "undefined" && (crypto = require$$0)) {
...
function getPixelRatio() {
  return Number.isFinite(window.devicePixelRatio) ? Math.ceil(window.devicePixelRatio) : 1;
}

function canvasToURL(canvas) {
  return new Promise((resolve, reject) => {
    canvas.toBlob(blob => {
      blob === null ? reject(new Error('cannot create blob')) : resolve(URL.createObjectURL(blob));
    });
  });
}
...
async function load(src, noSmoothing = false) {
  const image = await loadImage(src);
  const ratio = getPixelRatio();
  if (ratio !== 1 && noSmoothing) {
    const { width, height } = image;
    return makeCache((await makeImage(copyImage(image, width, height, false), width, height)));
  }
...
async function resize(id, width, height, noSmoothing = false) {
  return makeCache((await makeImage(copyImage(get(id), width, height, !noSmoothing), width, height)));
}

async function trim(id, left, right, top, bottom) {
  const image = get(id);
...
async function trim$1(root, id) {
  const id1 = await trim(id, 25, 50, 75, 100);
  const image1 = get(id1);
  root.appendChild(image1);
}

async function resize$1(root, id) {
  const id1 = await resize(id, 100, 100);
  const image1 = get(id1);
  root.appendChild(image1);

  const id2 = await resize(id, 1000, 1000);
  const image2 = get(id2);
  root.appendChild(image2);
}

async function rotate$1(root, id) {
  for (let degree = 30; degree <= 360; degree += 30) {
    const id1 = await rotate(id, Math.PI / 180 * degree);
    const image1 = get(id1);
    root.appendChild(image1);
  }
}
...
async function main() {
  ...
  const id1 = await load('./image.png');
  const image1 = get(id1);
  root.appendChild(image1);

  await trim$1(root, id1);
  await resize$1(root, id1);
  await rotate$1(root, id1);
}
